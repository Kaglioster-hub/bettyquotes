...
/* === SPORT_SYNC_V3_DELUXE === */
(function(){
  if(window.__BQS_V3) return; window.__BQS_V3=true;
  const chips = document.getElementById('chips');
  const select = document.getElementById('sportSelect');
  if(!chips || !select) return;

  // se non ci sono chip in DOM, costruiscile da config.json
  if(!chips.querySelector('.chip')){
    try{
      fetch('/public/config.json').then(r=>r.json()).then(d=>{
        if(!d || !d.sports) return;
        chips.innerHTML = d.sports.slice(0,12).map((s,i)=>
          `<span class="chip ${(select.value===s.key||(!select.value&&i===0))?'active':''}" data-key="${s.key}">${(s.name||'').split('•')[1]?.trim()||s.name}</span>`
        ).join('');
        wire();
      });
    }catch(e){}
  ...
  function setSport(key, from){
    ...
    if(from!=='chip'){ chips.querySelectorAll('.chip').forEach(c=> c.classList.toggle('active', c.dataset.key===key)); }
    try{ localStorage.setItem('bq_sport', key); }catch(e){}
    reloadCurrent();
  }

  // harden loadTop/loadAndRender: se 0 risultati, prova reseed demo e ricarica
  function harden(fn){
    if(typeof fn!=='function') return fn;
    return async function(...args){
      const out = await fn.apply(window,args);
      try{
        const list = document.getElementById('list');
        if(list && !list.children.length){
          await fetch('/api/reseed_demo');
          await fn.apply(window,args);
        }
      }catch(e){}
      return out;
    }

...
(()=>{ if(window.__BQS_SUPREME_CORE__) return; window.__BQS_SUPREME_CORE__=true;
  ...
  const apply=t=>{document.documentElement.setAttribute("data-theme",t); document.documentElement.classList.toggle("dark",t==="dark"); localStorage.setItem("theme",t); if(themeBtn){themeBtn.setAttribute("aria-pressed",String(t==="dark")); themeBtn.textContent=t==="dark"?"☾":"☀︎";}}
  ...
  // Popola select e crea chips se mancano (da config.json)
  (async function initSports(){
    if(sel && ![...sel.options].some(o=>o.value&&o.textContent.trim())){
      const cfg=await safeJSON("/public/config.json"); const sp=cfg?.sports||[]; if(sp.length){ sel.innerHTML=sp.map(s=>`<option value="${s.key}">${s.name}</option>`).join(""); }
    }
    if(chips && !chips.querySelector(".chip")){
      const cfg=await safeJSON("/public/config.json"); const sp=cfg?.sports||[]; const val=sel?.value || (sp[0]?.key||"");
      chips.innerHTML=sp.slice(0,12).map(s=>{const lab=(s.name||"").split("•")[1]?.trim()||s.name; const act=s.key===val?'active':''; return `<span class="chip ${act}" data-key="${s.key}">${lab}</span>`}).join("");
    }
    // wiring
    ...
    function reload(){ const isTop=document.getElementById("btnTop")?.dataset.active==="1"; if(isTop&&typeof window.loadTop==="function") window.loadTop(); else if(typeof window.loadAndRender==="function") window.loadAndRender("all"); }
    ...
    // post-render resync + fallback demo se lista vuota
  ...
  document.addEventListener("click",e=>{ const b=e.target.closest(".oddbtn"); if(!b)return;
